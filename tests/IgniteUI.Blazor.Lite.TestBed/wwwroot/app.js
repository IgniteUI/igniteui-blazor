...
function stringifyObject(object) {
    ...
    var result = JSON.stringify(object, function (key, value) {
        if (key && key.startsWith("$") || key.startsWith("_") || key == "name" || key == "externalObject" || key == "level" || value === null) {
...
function triggerEvent(eventName, arg) {
    ...
    target.dispatchEvent(new CustomEvent(eventName, Object.assign({
        bubbles: true,
        cancelable: false,
        composed: true,
        detail: arg || {},
...
function generateClientTmpl(name) {
    igRegisterScript(name, function TmplHandler(ctx) {
