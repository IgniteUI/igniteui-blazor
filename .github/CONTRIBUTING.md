...
# Workflow
When working on an issue for the Ignite UI for Blazor repository, you need to be aware of and to follow a correct status workflow. We have created a number of status labels in order to communicate well what the current status of a single issue/pull request is. The statuses are as follows:
...
5. `status: third-party-issue` this is the status of an issue that has been reviewed, has been determined to be an issue, but the root case is not in the Ignite UI for Blazor code. Example would be browser specific bugs caused by the particular browser's rendering or JavaScript engines, or an issue with the Blazor framework. Remove other statuses and place only this one if you're the one performing the investigation.
...
A npm package should be published each time we release new version of Ignite UI for Blazor. Its version should correspond to the version of the igniteui-Blazor npm package.
One could localize an application by importing the corresponding localized resource strings from the localization package (`igniteui-Blazor-i18n`) and use the methods described in the previous bullet to localize the whole application or part of it.

**Example:**

Inside app.module you can perform:
```ts
import { IgxResouceStringsJA } from ‘igniteui-Blazor-i18n’;
...
1. Describe in the appropriate section in the [CHANGELOG.md](https://github.com/IgniteUI/igniteui-blazor/blob/master/CHANGELOG.md)
