...
Before generating any theme code, inspect the project's host page (`wwwroot/index.html`, `App.razor`, or `_Host.cshtml`) and global CSS files (`wwwroot/css/app.css` or `wwwroot/css/site.css`). Look for an existing CSS theme `<link>` or `:root` CSS custom property overrides that already define a palette.
...
The MCP output for Blazor produces CSS custom property overrides (not Sass). Apply them in your global CSS file (e.g., `wwwroot/css/app.css`) or as a `:root` block.
...
3. **Generate** — call `create_component_theme(component, platform: "blazor", tokens)` passing only the tokens whose resolved value differs from the global theme. This produces scoped CSS with the minimal override set.
...
- Call `create_component_theme("grid", platform: "blazor", ...)` with only `{ "header-background": "<resolved token>", "content-background": "<resolved token>", "row-hover-background": "<resolved token>" }`

Apply the generated CSS in the component's `.razor.css` isolation file using `::deep` to scope it to the web component selectors.
...
- Use `@bind-Value` or `@bind-SelectedIndex` for two-way binding (not `[(ngModel)]`)
