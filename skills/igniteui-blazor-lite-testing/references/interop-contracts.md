...
- If the component already has a test class, change its base from `BlazorComponentTestBase` to `ComponentWithContractTestBase<Igb<Name>>`, add `using IgniteUI.Blazor.Tests.Interop;`, and add the `Contract` property at the top. Leave all existing facts untouched. `<Name>Tests` lives in `<Name>Tests.cs` — except for **child** components, whose suite may share the parent's file (`SelectItemTests` is in `SelectTests.cs`).
...
| `ComponentToJson(x, i)`, tag `"Component"`  | anything else            | nothing — the argument is dropped, so `arguments` arrives empty while `types` still lists the tag |
