...
namespace IgniteUI.Blazor.Controls
{
    ...
    public partial class IgbTree : BaseRendererControl
    {
        ...
        internal override void SerializeCore(RendererSerializer ser)
        {
            ...
            if (IsPropDirty("ToggleNodeOnClick"))
            { ser.AddBooleanProp("toggleNodeOnClick", this._toggleNodeOnClick); }
            if (IsPropDirty("Selection"))
            { ser.AddEnumProp("selection", this._selection); }
            if (IsPropDirty("SelectionChangedRef"))
            { ser.AddStringProp("selectionChangedRef", this._selectionChangedRef); }
            if (IsPropDirty("ItemExpandingRef"))
            { ser.AddStringProp("itemExpandingRef", this._itemExpandingRef); }
            if (IsPropDirty("ItemExpandedRef"))
            { ser.AddStringProp("itemExpandedRef", this._itemExpandedRef); }
            if (IsPropDirty("ItemCollapsingRef"))
            { ser.AddStringProp("itemCollapsingRef", this._itemCollapsingRef); }
            if (IsPropDirty("ItemCollapsedRef"))
            { ser.AddStringProp("itemCollapsedRef", this._itemCollapsedRef); }
            if (IsPropDirty("ActiveItemRef"))
            { ser.AddStringProp("activeItemRef", this._activeItemRef); }
