namespace IgniteUI.Blazor.Controls
{
    ...
    /// </summary>
    /// <remarks>
    /// <see cref="IgbNavDrawerItem"/> is only ever used inside <see cref="IgbNavDrawer"/>, and the parent's web component registers its children itself, so this module never needed a separate registration.
    /// The type and its members are kept so existing registrations keep compiling; they do nothing.
    /// </remarks>
    [Obsolete("Registering IgbNavDrawerItemModule is not required and has no effect. Remove it from your AddIgniteUIBlazor registration.")]
