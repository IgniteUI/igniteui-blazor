namespace IgniteUI.Blazor.Controls
{
    ...
    /// </summary>
    /// <remarks>
    /// <see cref="IgbTreeItem"/> is only ever used inside <see cref="IgbTree"/>, and the parent's web component registers its children itself, so this module never needed a separate registration.
    /// The type and its members are kept so existing registrations keep compiling; they do nothing.
    /// </remarks>
    [Obsolete("Registering IgbTreeItemModule is not required and has no effect. Remove it from your AddIgniteUIBlazor registration.")]
