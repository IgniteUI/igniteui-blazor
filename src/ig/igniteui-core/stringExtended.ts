...
export function isLetterOrDigit(target: string) {
...
// jscs:disable
var unicodeCategories: { [val: string]: string } = {
...
let unicode_hack_impl = (function () { //jscs:ignore requireCamelCaseOrUpperCaseIdentifiers
    ...
    */
    ...
    var firstLetters: { [val: string]: string } = {}, p;
    ...
    return function (regexpString: string | RegExp): RegExp {
        var modifiers = "";
...
export function unicode_hack(regexpString: string | RegExp): RegExp {
    return unicode_hack_impl(regexpString);
}

//jscs:enable
export function netRegexToJS(netPattern: string) {
    var jsPattern = "";
    var nextNETGroupIndex = 1;
    var nextJSGroupIndex = 1;
    var namedGroups: string[] = [ ];
    var nameToJSGroupIndexMap: { [name: string ]: number[] } = {};
    ...
    var nameToNetGroupIndexMap: { [name: string]: number } = {};
