...
export class Loader {
  ...
  public async request(module: string, cr: ComponentRenderer) {
    if (this._moduleSet.has(module) || this._loadingSet.has(module)) {
      return;
    }
    ...
    switch (module) {
      case 'TemplateContainerModuleModule':
        ...
        ModuleManager.register(IgcTemplateContainerModule);
      ...
      case 'WebCircularGradientModule': {
        ...
        let { WebCircularGradientDescriptionModule } =
          await import('igniteui-core/WebCircularGradientDescriptionModule');
      ...
      case 'WebCircularProgressModule': {
        ...
        let { WebCircularProgressDescriptionModule } =
          await import('igniteui-core/WebCircularProgressDescriptionModule');
  ...
  public static hasId(obj: any) {
    return obj.id || obj.name || (obj.tagName && Loader.hasContainerIgIdAttribute(obj));
  }
  ...
  private static isRoot(value: any, root: any) {
    ...
    if (value.i && value.i.nativeElement) {
  ...
  private static getName(value: any, root: any) {
    ...
    if (
      ...
      Loader.hasId(value) &&
      value.tagName &&
      value.tagName.toLowerCase().indexOf('igc-') == 0
    ) {
  ...
  private static getClone(value: any, root: any, seen: Map<any, any>) {
    ...
    if (
      ...
      value != undefined &&
      typeof value == 'object' &&
      !Loader.isRoot(value, root) &&
      (!hasName || mustPassByValue) &&
      !value.___cloned
    ) {
      ...
      if (inVal.$type && inVal.$type.name == 'Point') {
        value.x = inVal.x;
        value.y = inVal.y;
      } else if (!inVal.$type && !inVal._implementation && inVal.x !== undefined && inVal.y !== undefined) {
        value.___type = 'Point';
        value.x = inVal.x;
        value.y = inVal.y;
        ...
        value.width = inVal.width;
        value.height = inVal.height;
      } else if (!inVal.$type && !inVal._implementation && inVal.width !== undefined && inVal.height !== undefined) {
        value.___type = 'Size';
        value.width = inVal.width;
        value.height = inVal.height;
        ...
        for (var copyKey of getAllPropertyNames(inVal)) {
          if (
            copyKey == '$type' ||
            copyKey == '_implementation' ||
            copyKey == 'constructor' ||
            copyKey == 'seriesInternal' ||
            copyKey == 'prototype' ||
            copyKey == '__proto__' ||
            copyKey == 'externalObject' ||
            copyKey == 'i'
          ) {
            continue;
          }
